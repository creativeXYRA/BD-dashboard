...
# Set up logging for Cloud Run. This helps with debugging.
logging.basicConfig(level=logging.INFO)
logging.info("Starting Asymchem BD Dashboard application...")

# Get the port from the environment variable, as required by Cloud Run
...
# --------------------------------------------------------------------------------
# Step 2: Define Functions for Dynamic Elements
# --------------------------------------------------------------------------------
def create_network_elements(df_bd, leadership_data, bd_team_data):
    ...
    for person in all_people_list:
        ...
        node_class = 'bd_person' if person.get('company') and person['company'] not in ['Asymchem', 'BD Director', 'Sr. Director of BD'] else 'leader'
...
# --------------------------------------------------------------------------------
# Step 3: Initialize the App and Layout
# --------------------------------------------------------------------------------
# Initialize the app with Tailwind CSS from CDN
app = Dash(__name__, external_stylesheets=['https://cdn.jsdelivr.net/npm/tailwindcss@2.2.19/dist/tailwind.min.css'])

# `server` is a special variable that gunicorn looks for.
server = app.server
...
def update_bd_data(n_clicks, new_name, new_company, new_connections, current_data):
    if not n_clicks or n_clicks == 0 or not new_name or not new_company or not new_connections:
        raise dash.exceptions.PreventUpdate

    try:
        df_bd_current = pd.read_json(current_data, orient='split')
    except (json.JSONDecodeError, ValueError) as e:
        logging.error(f"Error decoding BD data from store: {e}")
        df_bd_current = pd.DataFrame(bd_data) # Fallback to initial data
...
# Step 5: Run the App
# --------------------------------------------------------------------------------
# This block ensures the app runs correctly on Cloud Run
if __name__ == '__main__':
    logging.info("Running server locally...")
